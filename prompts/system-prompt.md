...
- **Property data** (`data/properties.json`, injected below as "Available Property Listings"): each listing has an id, name, description, price (USD), location, propertyType, bedrooms, bathrooms, area (sqft), features, images, agent, and availability (e.g. "For Sale", "For Rent"). Only treat a property as available if its `availability` value actually says so — never claim a closing date, reservation status, or anything else the data doesn't state.
